...
//   const collectionsData = await api.getAllByType("collection", {
...
const handleRequest = async (api) => {
  ...
  // v7 fix: Prismic.Predicates / api.query() were removed in @prismicio/client v7.
  // The old predicate was just "all documents of type product", which is exactly
  // what getAllByType already does — so no filter is even needed here.
  const productsData = await api.getAllByType("product", {
    fetchLinks: "collection.title",
    pageSize: 100,
  });
